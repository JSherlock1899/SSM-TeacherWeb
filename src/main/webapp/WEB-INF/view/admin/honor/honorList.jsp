...
<div class="table-main col-md-12">
    <div class="col-md-4" >
        <ol class="breadcrumb">
            <li><a href="#">主页</a></li>
            <li><a href="#">查询</a></li>
            <li class="active"><a href="#">荣誉查询</a></li>
        </ol>
    </div>
    <div class="row">
        <div class="col-md-11 col-md-offset-1 ">
            <div class="col-md-10 button-div form-inline">
                ...
                <form action="<%=request.getContextPath()%>/honor/export.do?&cname=<%=condition.getCname()%>
            ...
            <div class="row">
                ...
                <div>
                    <ul class="pagination">
                        <li><a href="${pageContext.request.contextPath}/honor/findHonor.do?pn=1&cname=<%=condition.getCname()%>
                        ...
                        <li>
                            <c:if test="${pageInfo.hasPreviousPage}">
                                <a href="${pageContext.request.contextPath}/honor/findHonor.do?pn=${pageInfo.pageNum-1}&cname=<%=condition.getCname()%>
                        ...
                        <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                            <c:if test="${page_num == pageInfo.pageNum}">
                                <li class="active"><a href="${pageContext.request.contextPath}/honor/findHonor.do?pn=1&cname=<%=condition.getCname()%>
                            ...
                            <c:if test="${page_num != pageInfo.pageNum}">
                                <li><a href="${pageContext.request.contextPath}/honor/findHonor.do?pn=${page_num}&cname=<%=condition.getCname()%>
                            &dname=<%=condition.getDname()%>&starttime=<%=condition.getStarttime()%>&endtime=<%=condition.getEndtime()%>&tname=<%=condition.gettname()%>">${page_num}</a></li>
                            </c:if>
                        </c:forEach>
                        <!--下一页-->
                        ...
                        <li><a href="${pageContext.request.contextPath}/honor/findHonor.do?pn=${pageInfo.pages}&cname=<%=condition.getCname()%>
...
<script type="text/javascript">
    ...
    $(document).on("click","#skipPage",function () {
        ...
        if(!isNaN(pageVal)){
            window.location.href = "${pageContext.request.contextPath}/honor/findHonor.do?pn=" + pageVal + "&cname=<%=condition.getCname()%>\n" +
                "&dname=<%=condition.getDname()%>&starttime=<%=condition.getStarttime()%>&endtime=<%=condition.getEndtime()%>&tname=<%=condition.gettname()%>";
        }else {
            alert('请输入数字！')
        }
