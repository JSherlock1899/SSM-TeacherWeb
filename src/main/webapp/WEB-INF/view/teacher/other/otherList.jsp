    ...
    <div class="row">
        <div class="col-md-11 col-md-offset-1 ">
            ...
            <div class="row">
                ...
                <div>
                    <ul class="pagination">
                        <li><a href="${pageContext.request.contextPath}/teacher/findOther.do?pn=1&tsn=<%=tsn%>">首页</a></li>
                        <!--上一页-->
                        <li>
                            <c:if test="${pageInfo.hasPreviousPage}">
                                <a href="${pageContext.request.contextPath}/teacher/findOther.do?pn=${pageInfo.pageNum-1}&tsn=<%=tsn%>" aria-label="Previous">
                        ...
                        <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                            <c:if test="${page_num == pageInfo.pageNum}">
                                <li class="active"><a href="${pageContext.request.contextPath}/teacher/findOther.do?pn=1&tsn=<%=tsn%>">${page_num}</a></li>
                            </c:if>
                            <c:if test="${page_num != pageInfo.pageNum}">
                                <li><a href="${pageContext.request.contextPath}/teacher/findOther.do?pn=${page_num}&tsn=<%=tsn%>">${page_num}</a></li>
                            </c:if>
                        </c:forEach>
                        <!--下一页-->
                        <li>
                            <c:if test="${pageInfo.hasNextPage}">
                                <a href="${pageContext.request.contextPath}/teacher/findOther.do?pn=${pageInfo.pageNum+1}&tsn=<%=tsn%>"
                                   aria-label="Next">
                                    <span aria-hidden="true">»</span>
                                </a>
                            </c:if>
                        </li>
                        <li><a href="${pageContext.request.contextPath}/teacher/findOther.do?pn=${pageInfo.pages}&tsn=<%=tsn%>">尾页</a></li>
                    </ul>
                    <!--文字信息-->
...
<script type="text/javascript">
    ...
    $(document).on("click","#skipPage",function () {
        ...
        if(!isNaN(pageVal)){
            window.location.href = "${pageContext.request.contextPath}/teacher/findOther.do?pn" + pageVal + "&tsn=<%=tsn%>";
        }else {
            alert('请输入数字！')
        }
    })
