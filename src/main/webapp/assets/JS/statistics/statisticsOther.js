...
$(function() {
    ...
    var myChart = echarts.init(document.getElementById('other_bar'));
    // 分别获取从后台传过来的数据
    var json = $('#json').val();
    var cname = $('#cname').val();
    ...
    if(cname == null || cname == "null"){
        for (var i = 0;i<json.length;i++) {
            ...
            valueArr.push(json[i].count) // 添加其他成果数
            jsonArr.push({
                name: json[i].Cname,
                value: json[i].count
            })
    ...
    // 绘制图表。
    // 饼图
    var myChart = echarts.init(document.getElementById('other_pie'));
    var pie_option = {
        title : {
            text : text,
            x : 'center'
...
$(function () {
    var myChart = echarts.init(document.getElementById('other_broken_line'));
    var cname = $('#cname').val();
    var jsonList = $('#jsonList').val();
