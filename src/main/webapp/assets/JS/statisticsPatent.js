...
$(function() {
	...
	var myChart = echarts.init(document.getElementById('patent_bar'));
	// 分别获取从后台传过来的数据
	var json = $('#json').val();
	var grade = $('#grade').val();
	...
	var valueArr = new Array();// 专利数
	var jsonArr = [];
	for ( var key in json) {
		keyArr.push(key); // 添加学院名
		valueArr.push(json[key]) // 添加专利数
		jsonArr.push({
			name : key,
			value : json[key]
		})
	...
	// 绘制图表。
	// 饼图
	var myChart = echarts.init(document.getElementById('patent_pie'));
	var pie_option = {
		title : {
			text : text,
			x : 'center'
	...
	if (grade == "1") {		// 校级
		...
		var myChart = echarts.init(document
				.getElementById('patent_broken_line'));
		option = {
			title : {
				text : '折线图堆叠'
			},
		...
		var myChart = echarts.init(document.getElementById('patent_broken_line'));
		 //院级
		 var option = {
		 title: {
		 text: '折线图堆叠'
